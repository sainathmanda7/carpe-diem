...
const DriftWall = ({
  ...
  const columnMeta = useMemo<ColumnMeta[]>(() => {
    ...
    return columnItems.map(col => {
      ...
      const copies = Math.max(3, Math.ceil((containerHeight * 2.2) / copyHeight) + 2);
  ...
  useLayoutEffect(() => {
    ...
    const ro = new ResizeObserver(([entry]) => {
      if (entry) {
        if (entry.contentRect.height) setContainerHeight(entry.contentRect.height);
  ...
  const applyPlaneTransform = useCallback(
    (px: number, py: number) => {
      ...
      plane.style.transform =
        `translate(calc(-50% + ${planeOffsetX}px), -50%) scale(${scale}) ` +
        `rotateX(${tilt + py}deg) rotateY(${turn + px}deg) rotateZ(${roll}deg) ` +
        `translateZ(${-depth}px)`;
    },
    [tilt, turn, roll, depth, scale, planeOffsetX]
  );
